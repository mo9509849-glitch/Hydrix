/* ============================================================
   ...
   الحساسات:
     ...
     DHT22          → GPIO 4
     Relay (المضخة) → GPIO 23
       تشغيل: OUTPUT + LOW  |  إيقاف: تحويل البين إلى INPUT (عائم)
       بدل الاعتماد على HIGH، لأن جهد HIGH من ESP32 (3.3V) لا يكفي
       دائماً لإيقاف بعض لوحات الريلاي التي تعمل بمنطق 5V.
...
// ملاحظة: الإيقاف بقى بتحويل البين لـ INPUT مش بـ RELAY_OFF،
// فالتعريف ده بقى مستخدم بس عند التشغيل.
#define RELAY_ON  LOW
...
bool pumpOn = false;
bool smartActive = false;   // الوضع الافتراضي: الري الذكي متوقف حتى يفعّله المستخدم
...
void setPump(bool on) {
  ...
  if (on) {
    ...
    pinMode(PIN_RELAY, INPUT);   // نسيب البين عائم بدل ما ندفعه HIGH
  }
}
...
class RxCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *ch) override {
    ...
    if (cmd == "ON") {
      ...
      // لازم يتفحص قبل startsWith("T") العام، وإلا TH: هتقع فيه غلط
      applyThresholds(cmd);
...
void setup() {
  Serial.begin(115200);
  pinMode(PIN_RELAY, INPUT);   // ابدأ بحالة إيقاف (عائم)
  dht.begin();
