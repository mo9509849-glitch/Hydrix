...
const state = {
  ...
  selectedStartCrop: "",
  irrigationCheck: null,
  irrigationCheckTimer: null,
  pumpFault: null,
};
...
/* ---------------- الراوتر ---------------- */
const SCREENS = ["welcome", "crop", "dashboard", "manual", "timed", "smart", "plant", "camera", "history", "settings", "about"];
...
$("btnStart").addEventListener("click", () => {
  state.selectedStartCrop = state.crop || "";
  document.querySelectorAll(".start-crop-chip").forEach((c) =>
    c.classList.toggle("selected", c.dataset.crop === state.selectedStartCrop)
  );
  $("btnContinueCrop").disabled = !state.selectedStartCrop;
  $("btnContinueCrop").textContent = state.selectedStartCrop ? "متابعة إلى لوحة التحكم" : "اختر محصولًا للمتابعة";
  location.hash = "crop";
});

document.querySelectorAll(".start-crop-chip").forEach((chip) => chip.addEventListener("click", () => {
  state.selectedStartCrop = chip.dataset.crop;
  document.querySelectorAll(".start-crop-chip").forEach((c) =>
    c.classList.toggle("selected", c === chip)
  );
  $("btnContinueCrop").disabled = false;
  $("btnContinueCrop").textContent = "متابعة إلى لوحة التحكم";
}));

$("btnContinueCrop").addEventListener("click", () => {
  if (!state.selectedStartCrop) return;
  state.crop = state.selectedStartCrop;
...
function startIrrigationCheck(source) {
  clearTimeout(state.irrigationCheckTimer);
  state.irrigationCheck = {
    startMoisture: Number(state.moisture),
    startedAt: Date.now(),
    source,
  };
  state.pumpFault = null;
}

function scheduleIrrigationCheck() {
  clearTimeout(state.irrigationCheckTimer);
  if (!state.irrigationCheck) return;
  const check = state.irrigationCheck;
  state.irrigationCheckTimer = setTimeout(() => {
    const rise = Number(state.moisture) - check.startMoisture;
    if (rise < MIN_MOISTURE_RISE) {
      state.pumpFault = {
        rise,
        checkedAt: Date.now(),
        message: "⚠️ عطل محتمل: لم تزد رطوبة التربة بشكل كافٍ بعد الري. افحص المضخة وخط المياه لاحتمال وجود عطل أو انسداد.",
      };
      toast(state.pumpFault.message, 7000);
      notify(state.pumpFault.message);
      logEvent("fault", "pump_check", null);
    } else {
      state.pumpFault = null;
      toast(`تم فحص الري بنجاح — زادت رطوبة التربة ${rise.toFixed(1)}%`, 3500);
    }
    state.irrigationCheck = null;
    render();
  }, PUMP_CHECK_DELAY);
}
...
// تشخيص مباشر على الشاشة: أي خطوة تفشل في الاتصال هتظهر كـ toast فورًا
// حتى لو الموبايل مش متوصل بلابتوب لفتح الـ Console
HydrixBLE.onLog = (level, msg) => {
  if (level === "error" || level === "warn") toast(msg, 4500);
};
...
/* ---------------- التحكم الصوتي ---------------- */
let startVoiceCommand = () => {}; // يتفعّل جوه initVoice، وتستخدمه كلمة التفعيل "Hi Hydrix"
...
(function initVoice() {
  ...
  fab.addEventListener("click", begin);
  startVoiceCommand = begin; // متاحة لكلمة التفعيل الصوتية
})();

/* ---------------- كلمة التفعيل "Hi Hydrix" ---------------- */
(function initWakeWord() {
  const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
  if (!SR) return; // المتصفح مش بيدعم — الكارت يفضل مخفي
  ...
  const matchesWake = (text) => WAKE_PATTERNS.some((p) => p.test(text));

  let wakeEnabled = localStorage.getItem("hydrix_wake") === "1";
  let wakeRec = null;
  let active = false;

  function updateWakeUI() {
    $("wakeWordText").textContent = wakeEnabled ? 'تفعيل «Hi Hydrix»: مفعل' : 'تفعيل «Hi Hydrix»: غير مفعل';
    $("btnWakeWord").classList.toggle("chip-on", wakeEnabled);
  }

  function startWakeListening() {
    if (!wakeEnabled || active) return;
    try {
      wakeRec = new SR();
      wakeRec.lang = "ar-EG";
      ...
      wakeRec.onresult = (e) => {
        for (let i = e.resultIndex; i < e.results.length; i++) {
          ...
          if (matchesWake(text)) {
            stopWakeListening();
            toast('سمعتك! قول أمرك دلوقتي…');
            setTimeout(() => startVoiceCommand(), 400);
            return;
          }
        }
      };
      wakeRec.onerror = () => { /* no-speech وغيرها — هيعاد المحاولة في onend */ };
      wakeRec.onend = () => {
        active = false;
        if (wakeEnabled) setTimeout(startWakeListening, 400); // إعادة تشغيل تلقائية
      };

      wakeRec.start();
      active = true;
    } catch { /* في حالة تشغيل مسبق */ }
  }

  function stopWakeListening() {
    if (wakeRec) {
      wakeRec.onend = null;
      try { wakeRec.stop(); } catch { /* متوقف بالفعل */ }
    }
    active = false;
  }
  ...
  $("btnWakeWord").addEventListener("click", () => {
    ...
    localStorage.setItem("hydrix_wake", wakeEnabled ? "1" : "0");
    updateWakeUI();
    if (wakeEnabled) {
      toast('تم تفعيل الاستماع — نادي «Hi Hydrix» في أي وقت');
      startWakeListening();
    } else {
      toast("تم إيقاف الاستماع لكلمة التفعيل");
      stopWakeListening();
    }
  });
